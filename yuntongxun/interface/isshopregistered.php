...
$isshopregistered=new IsShopRegistered();
if(isset($_POST['phone'])){
	...
	if($serversign==$signature){
		$result=$isshopregistered->isRegisteredPhone($phone);
		if($result=="1"){
		...
		echo $status;
		//写入到数据库
		$isshopregistered->writeShopCheckCodeToDB($phone, $checkcode,$time);
	}else{
...
exit;
echo $isshopregistered->isRegisteredPhone("k8a3fyEC5VXeh08N3JNRng==");
?>