...
$aplipayapi=new AliPayApi();
$dalidata=$aplipayapi->getDaliAccount($shopid);
...
/**************************请求参数**************************/
        ...
        //必填，不能修改
        //服务器异步通知页面路径
        $notify_url = "http://shop.meijiemall.com/houtai/shop/directpay/notify_url.php";
        //需http://格式的完整路径，不能加?id=123这类自定义参数
        //页面跳转同步通知页面路径
        $return_url = "http://shop.meijiemall.com/houtai/shop/directpay/return_url.php";
...
if(!empty($agentaccount)){
	...
	$parameter = array(
			...
			"royalty_type"=>"10",
			"royalty_parameters"=>$agentaccount."^".(0.8*$total_fee)."^账单",
	);
}else{
	$parameter = array(
			"service" => "create_direct_pay_by_user",
