...
class HistoryOrder{
	public function getHistoryLists($shopid,$uid,$date=NULL){
		return PRINT_InterfaceFactory::createInstanceDoWorkerTwoDAL()->getHistoryLists($shopid,$uid,$date);
	}
}
...
if(isset($_GET['uid'])){
	...
	$shopid=$_GET['shopid'];
	$tabid=$_GET['tabid'];
	$arr=$historyorder->getHistoryLists($shopid,$uid,$date);	
}
                    ...
                    <div class="histinfo">
                        ...
                        ?>
                         <li onclick="window.location.href='./order-details.php?billid=<?php echo $val['billid'];?>&shopname=<?php echo $val['shopname'];?>'">
                           <div class="time"><?php echo date("m-d H:i",$val['time']);?></div>
...
<script type="text/javascript">
    ...
    for(var i=0;i<historyOrders.length;i++){
        historyOrder+='<li><div class="time">'+historyOrders[i].time+'</div>'+
            '<div class="history-count">'+
              '  <p>流水号:</p>'+
              '  <p class="mondy-total">消费共计：'+historyOrders[i32].mondyTotal+'</p>'+
           ' </div>'+
           ' <div class="pay-button">'+
