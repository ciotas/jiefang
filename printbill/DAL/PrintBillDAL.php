...
// 		$getexistbillnum=$this->getTheBillSortNum($inputdarr['shopid']);
	...
	public function tobeConsumeList($inputdarr,$paymethod,$paymoney) {
		...
		if(!empty($inputdarr['serverfee'])){$serverfee=$inputdarr['serverfee'];}else{$serverfee="0";}
		if(!empty($inputdarr['servermoney'])){$servermoney=$inputdarr['servermoney'];}else{$servermoney="0";}
		...
		foreach ($result as $key=>$val){
			...
			$arr[]=array(
					...
					"paidorderrequest"=>$inputdarr['paidorderrequest'],
