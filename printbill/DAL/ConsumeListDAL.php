...
class ConsumeListDAL implements IConsumeListDAL{ 
	...
	public function createContentHtml($arr,$deviceno,$devicekey) {
		...
		$workdal=new WorkerDAL();
		$shopinfo=$workdal->getShopInfo($arr['shopid']);
		$foodlist="";
...
// 		$billnum=$this->getBillnumToday($shopid);
			...
			if($arr['discountval']!="100"){
				if($arr['discountval']>100){
					...
					if($arr['discountmode']=="all"){
					    if(empty($arr['serverfee'])){
					        $discountmoney+=ceil($totalmoney*(1-$arr['discountval']/100));
					        $disaccountdetails.=$arr['discountval']."折 优惠".ceil($totalmoney*(1-$arr['discountval']/100))."<BR>";
					    }else{
					        $discountmoney+=ceil($totalmoney*(1+$arr['serverfee']/100)*(1-$arr['discountval']/100));
					        $disaccountdetails.=$arr['discountval']."折 优惠".ceil($totalmoney*(1+$arr['serverfee']/100)*(1-$arr['discountval']/100))."";
					    }
					}else{
					    if(empty($arr['serverfee'])){
					        $discountmoney+=ceil($discountfoodmoney*(1-$arr['discountval']/100));
					        $disaccountdetails.=$arr['discountval']."折 优惠".ceil($discountfoodmoney*(1-$arr['discountval']/100))."<BR>";
					    }else{
					        $discountmoney+=ceil($discountfoodmoney*(1+$arr['serverfee']/100)*(1-$arr['discountval']/100));
					        $disaccountdetails.=$arr['discountval']."折 优惠".ceil($discountfoodmoney*(1+$arr['serverfee']/100)*(1-$arr['discountval']/100))."";
					    }
					}
				}
			}
...
// 			}
			...
			if(!empty($arr['tabname'])){$tabname='台号：'.$arr['tabname'];}else{$tabname="";}
			...
			if(empty($arr['cusnum'])){
			    ...
			    $cusnum='   人数：'.$arr['cusnum'];
			}
			if($arr['takeout']=="1"){
			    $cusbillnum=$this->getCUsDownbillNum($arr['shopid'], $arr['uid']);
			    $orderInfo.="此顾客第".$cusbillnum."单<BR>";
			}
			$orderInfo.=$tabname.$cusnum.'<BR>';
			...
			$orderInfo.='应收小计：'.sprintf('%.0f',$totalmoney).'';
			if($arr['deposit']=="1" && !empty($depositmoney)){
				$orderInfo.="(包含押金：￥".$depositmoney.")";
			}
			$orderInfo.="<BR>";
			if(!empty($arr['serverfee'])){
			    $orderInfo.="服务费［".$arr['serverfee']."%］ 金额".$arr['servermoney']."<BR>";
			}
			if(!empty($disaccountdetails)){
// 				$orderInfo.='费用详细：'.$disaccountdetails.'<BR>';
			    $orderInfo.=''.$disaccountdetails.'<BR>';
// 				$orderInfo.='优惠总额：￥'.$discountmoney.'<BR><BR>';
			...
			if(!empty($arr['paidorderrequest'])){
			    $orderInfo.="<B>备注：".$arr['paidorderrequest']."</B><BR>";
			}
			$orderInfo .='---------------------------------------------<BR>';
	...
	public function createSmallContentHtml($arr,$deviceno,$devicekey){
		...
		foreach ($arr['food'] as $kf=>$valf){
			...
			if(!empty($valf['cooktype'])){
			    $cooktype="(".$valf['cooktype'].")";
			}
			$foodlength=(strlen($valf['foodname'].$cooktype.$donatestr) + mb_strlen($valf['foodname'].$cooktype.$donatestr,'UTF8'))/2;
			if($foodlength>12){
				$foodname=$valf['foodname'].$cooktype.$donatestr."<BR>";
			}else{
				$foodname=$this->getStableLenStr($valf['foodname'].$cooktype.$donatestr, 12);
			}
...
// 			$orderInfo.='优惠总额：￥'.$discountmoney.'<BR><BR>';
		...
		if(!empty($oneshopinfo)){
			$orderInfo.='收货地址：'.$oneshopinfo['prov']."省".$oneshopinfo['city']."市".$oneshopinfo['dist'].$oneshopinfo['road'].'<BR>';
			if(!empty($oneshopinfo['shopname'])){
			    $orderInfo.='店名：'.$oneshopinfo['shopname'].'<BR>';
			}
			if(!empty($oneshopinfo['author'])){
	...
	/**
	 * {@inheritDoc}
	 * @see IWorkerTwoDAL::getCUsDownbillNum()
	 */
	public function getCUsDownbillNum($shopid, $uid)
	{
	    // TODO Auto-generated method stub
...
// 		$shopinfo=$workdal->getShopInfo($arr['shopid']);
		...
		if($arr['discountval']!="100"){
			if($arr['discountval']>100){
				...
				if($arr['discountmode']=="all"){
					$discountmoney+=ceil($totalmoney*(1-$arr['discountval']/100));
					$disaccountdetails.="打".$arr['discountval']."折 优惠￥".ceil($totalmoney*(1-$arr['discountval']/100))."，";
				}else{
					$discountmoney+=ceil($discountfoodmoney*(1-$arr['discountval']/100));
					$disaccountdetails.="打".$arr['discountval']."折，优惠￥".ceil($discountfoodmoney*(1-$arr['discountval']/100))."，";
				}
			}
		}
		if(!empty($arr['serverfee'])){
		    $disaccountdetails.="服务费[".$arr['serverfee']."%] 金额".$arr['servermoney']."，";
		}
		//优惠券
		if(!empty($arr['ticketway'])&&!empty($arr['ticketnum'])&&!empty($arr['ticketval'])){
			$ticketname=$this->getOneCounponType($arr['ticketway']);
		...
		elseif($arr['paystatus']=="unpay"){
			if(!empty($arr['qrcode'])){
				$paytypestr.="总计：￥".$arr['shouldpay']." <BR>
						用支付宝客户端扫一扫，立即买单！<BR><QRcode>https://qr.alipay.com/".$arr['qrcode']."</QRcode>";
				$title="预结单";
			}
		}elseif($arr['cashmoney']=="0"&&empty($arr['alipay'])&&empty($arr['signername'])&&empty($arr['freename'])){
...
// 			$orderInfo.='优惠总额：￥'.$discountmoney.'<BR><BR>';
	...
	public function createPrediscountHtml($arr,$deviceno,$devicekey){
		...
		$discountmoney=0;
		if(!empty($arr['serverfee'])){
		    $disaccountdetails.="服务费［".$arr['serverfee']."%］小计".$arr['servermoney']."<BR>";
		}
		...
		if($predata['discountval']!="100"){
			if($predata['discountval']>100){
				if($predata['allcount']=="1"){
					$disaccountdetails.="服务费收￥".floor($totalmoney*(($predata['discountval']/100)-1))."<BR>";
				}else{
					$disaccountdetails.="服务费收￥".floor($discountfoodmoney*(($predata['discountval']/100)-1))."<BR>";
				}
		
			}else{
			     if($predata['discountval']!=100 && !empty($predata['discountval'])){
			        if($predata['allcount']=="1"){
			            if(empty($arr['serverfee'])){
			                $discountmoney+=ceil($totalmoney*(1-$arr['discountval']/100));
			                $disaccountdetails.=$arr['discountval']."折 优惠".ceil($totalmoney*(1-$arr['discountval']/100))."<BR>";
			            ...
			            if(empty($arr['serverfee'])){
			                ...
			                $discountmoney+=ceil($discountfoodmoney*(1+$arr['serverfee'])*(1-$arr['discountval']/100));
			                $disaccountdetails.=$arr['discountval']."折 优惠".ceil($discountfoodmoney*(1+$arr['serverfee']/100)*(1-$arr['discountval']/100))."";
			            }
			        }
			    }
...
// 			$orderInfo.='优惠总额：￥'.$discountmoney.'<BR><BR>';
		...
		$orderInfo .='---------------------------------------------<BR>';
		$orderInfo.='总计：'.$predata['shouldpay'].'<BR><BR>';
		$orderInfo.='(＊烟、酒类不打折，不收服务费)<BR>';
		$orderInfo.='<BR>';
		if(!empty($shopinfo)){
		    $orderInfo.='本店地址：'.$shopinfo['city']."市".$shopinfo['district']."区".$shopinfo['road'].'<BR>';
		    $orderInfo.='订餐电话：'.$shopinfo['servicephone'].'<BR>';
		}
		$orderInfo.='<BR>';
		$orderInfo.='<CB>欢 迎 惠 顾 </CB><BR>';
		$orderInfo.='打印时间：'.date('Y-m-d H:i:s',time()).'<BR>';
	...
	public function createPrediscountSmallHtml($arr,$deviceno,$devicekey){
		...
		$discountmoney=0;
		if(!empty($arr['serverfee'])){
		    $disaccountdetails.="服务费［".$arr['serverfee']."%］金额".$arr['servermoney']."<BR>";
		}
		if(!empty($predata['clearmoney'])){$discountmoney+=$predata['clearmoney'];$disaccountdetails.="抹零￥".$predata['clearmoney']."，";}
		if($predata['discountval']!="100"){
			if($predata['discountval']>100){
