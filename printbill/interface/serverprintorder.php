...
class ServerPrintBill{
	...
	public function updateBillCusnum($billid,$cusnum){
		PRINT_InterfaceFactory::createInstanceDoWorkerDAL()->updateBillCusnum($billid, $cusnum);
	}
...
if(isset($_POST['shopid']) && isset($_POST['uid'])){
	...
	$tabid=$_POST['tabid'];
	 if($shopid=="5539b36816c109ec748b4640"){
                file_put_contents("/var/www/html/printbill/log/serverbill.log", $tabid."_".time()."\r\n",FILE_APPEND);
        }
...
// 					$tabname=$newtabarr['tabname'];
			...
			if(!empty($billid)){//加菜
				...
				$serverprintbill->updateBillCusnum($billid, $cusnum);
...
// 			if(!empty($billid)){//加菜// && !empty($billid)
// 				$foodarr=array_merge($foodarr,$packarr);
// 				$inputarr['food']=$foodarr;
// 				$billarr=$inputarr;
// 			}else{
...
echo $serverprintbill->getBillnumToday($shopid);
exit;
$foodarr=array(
	"0"=>array(
...
$paystatus="paid";
$billid="558a4fd95bc109d33e8b45bb";
$billarr=$serverprintbill->getOneBillInfoByBillid($billid);
$paymoney="98";
if($paystatus=="paid"){
	$paymethod="serverpay";
