...
if(isset($_POST['shopid'])){
    ...
    $sessionresult=$commonpay->getTokenStatus($shopid);
    if(is_array($sessionresult)&&!empty($sessionresult)){
        $token=$sessionresult['token'];
        $serversign=strtoupper(md5($shopid.$billid.$clearmoney.$othermoney.$cashmoney.$unionmoney.$vipmoney.$discountval.$ticketval.$ticketnum.$ticketway.$timestamp.$token));
        if($serversign==$signature){//验证通过
            switch ($sessionresult['status']){
                case "valid":$session="";break;
                case "invalid":$session=session_id();$commonpay->updateShopSession($shopid,$session);break;
            }
            //打印出消费清单
            //收银员
            //得到商店名
