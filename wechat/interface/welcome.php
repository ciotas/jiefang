...
class Welcome{
	...
	public function getTabNum($tabid){
	    return Wechat_BLLFactory::createInstanceWechatBLL()->getTabNum($tabid);
	}
}
...
if(isset($_REQUEST['shopid'])){
	...
	if($_REQUEST['tabid']){
	    $tabName = $welcome->getTabNum($_REQUEST['tabid']);
	}
// 	$shopid = '5748a2b21a156fdc138b456f';
...
.welBanner{height:70%;background: url("../media/images/add.jpg") 50% 50% / 90% no-repeat;background-size:100% 100%;} 
 ...
 <div class="fullele welcome" style="overflow:visible;">
    <div class="welcomehd">
        <!--活动专题图片-->
        <div class="welBanner J_welBanner" ></div>
        <div class="titleer" style="margin-top:0.5rem">
            <h1><?php if(!empty($theshoparr)){echo $theshoparr['shopname']; if(!empty($tabName)){ echo "({$tabName}号桌)";}}?></h1>
<!--             <h3></h3> -->
        </div>
...
<!--         <div class="coupon"></div> -->
        <div class="mainer" style="margin:0.5rem auto;">
            <div class="open-wx">
                <div class="order-main" style="width:80%;overflow:hidden;margin:0 auto;">
                        <span class="button none J_Start" style = "width:60%;height:30px;line-height:30px;border-width:1px;text-agline:center;margin:0 auto;">开启自助点餐</span>
                </div>
            </div>
        </div>
        ...
        <div class="we-footer" style="color:#595857;text-align:center;line-height:20px;">
            ...
            <i><small style="color:#727171;opacity:0.8;">电话：<?php echo $theshoparr['servicephone'];?> </small></i></span>
