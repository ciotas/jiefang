...
class NotifyUrl{
	...
	public function sendDownbillMsg($shopid,$billid,$paymoney){
	    PRINT_InterfaceFactory::createInstanceDoWorkerTwoDAL()->sendDownbillMsg($shopid,$billid,$paymoney);
	}
}
...
if($verify_result) {//验证成功
	...
	$dist=$bodyarr['dist'];
	$road=$bodyarr['road'];
	$shopname=$bodyarr['shopname'];
	$contact=$bodyarr['contact'];
	$phone=$bodyarr['phone'];
	$picktime=$bodyarr['picktime'];
	$from=$bodyarr['from'];
	$orderrequest=$bodyarr['orderrequest'];
	$dicountfee=$bodyarr['dicountfee'];
	$distributefee=$bodyarr['distributefee'];
	...
	$cusnum=isset($beforeinfo['cusnum'])?$beforeinfo['cusnum']:0;
...
//     	file_put_contents("/var/www/html/log.txt", json_encode($inputarr));
    	...
    	$notifyurl->intoBillShopinfo($shopinfoarr,"billshopinfo");
...
//     	$notifyurl->delPrebillByBillid($billid);
	...
	if($from=="takeout"){
	    $notifyurl->sendDownbillMsg($shopid,$normalbillid,$paymoney);
	}
}
else {
    //验证失败
