...
require_once (_ROOT.'takeoutwechat/IDAL/IWechatDAL.php');
...
// 		echo $url;
        ...
        $url = ROOTURL."takeoutwechat/interface/welcome.php?uid=$uid&shopid=$shopid&tabid=$tabid&type=$type";
		$this->write_logs("[authorizeCode]: redirect_url=".$url);
		return $url;
// 		header("location: ".$url);
	}
	...
	public function getShopinfo($shopid) {
		...
		$oparr=array("shopname"=>1,"notice"=>1);
		$result=DALFactory::createInstanceCollection(self::$shopinfo)->findOne($qarr,$oparr);
		if(!empty($result)){
			$arr=array("shopname"=>$result['shopname'],"notice"=>$result['notice']);
		}
		return $arr;
	}
	/* (non-PHPdoc)
