...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	{
	    $xml = $this->createXml();
        $this->write_logs("xml123 = ".$xml);
        $this->write_logs("xml123 = ".$this->url);
		$this->response = $this->postXmlCurl($xml,$this->url,$this->curl_timeout);
        $this->write_logs("xml123response = ".$this->response);
		return $this->response;
	}
    
    function write_logs($content = '') {
        $date = date('Y-m-d');
...
class JsApi_pub extends Common_util_pub
{
	...
	function getOpenid()
	{
        ...
        $this->write_logs('url length = '.strlen($url));
        $this->write_logs('url = '.$url);
        //初始化curl
       	$ch = curl_init();
		//设置超时
