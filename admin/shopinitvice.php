...
$shopinitvice=new ShopinitVice();
$title="商家配置";
$menu="manage";
$clicktag="shopinitvice";
require_once ('header.php');
$zonearr=$shopinitvice->getBusinessZoneData();
$arr=$shopinitvice->getAllOnLineShop();
// print_r($arr);exit;
?>
<script>
var xmlHttp
...
function addZoneTag(shopid){
	obj=document.getElementById("busi_zoneid");
	var index = obj.selectedIndex; 
	var busi_zoneid = obj.options[index].value; // 选中值
	xmlHttp=GetXmlHttpObject()
	if (xmlHttp==null)
	  {
	  alert ("Browser does not support HTTP Request")
	...
	var url="./interface/addbusi_zoneid.php"
	url=url+"?shopid="+shopid
					...
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption"><i class="icon-credit-card"></i>商家配置</div>
								<div class="tools">
								</div>
							</div>
									...
									<?php foreach ($arr as $key=>$val){?>
										<tr>
											...
											<div class="control-group">
													<div class="controls">
														<select class="small m-wrap" tabindex="3" id="busi_zoneid" name="busi_zoneid" onchange="addZoneTag('<?php echo $val['shopid'];?>')">
															<option value="0">---未选择---</option>
															<?php foreach ($zonearr as $zkey=>$zval){?>
															<option value="<?php echo $zval['busi_zoneid'];?>" <?php if($zval['busi_zoneid']==$val['busi_zoneid']){echo "selected";}?>><?php echo $zval['busi_zonename'];?></option>
															<?php }?>
														</select>
													</div>
												</div>
