...
$odername=$_GET['subject'];
$paymoney=$_GET['total_fee'];
switch (strval($paymoney)){
	case "300":$buytype="permonth";break;
	case "1600":$buytype="perhalfyear";break;
	case "3000":$buytype="peryear";break;
}
$income=0;
if($verify_result) {//验证成功
	...
	if(!empty($agentaccount)){
		...
		$returnurl->addToDaliAccount($shopid, $income);//添加到代理商账号
	}
...
$endtime=$returnurl->getShopuseaccountEndtime($shopid);
switch ($buytype){
	case "permonth":
...
//购买记录
$buyid=$returnurl->buyAccountRecord($inputarr);
$returnurl->addShopUseAccount($shopaccountarr);
