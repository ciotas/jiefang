		  ...
		  if(returnnum){
			  if(returnnum<=0 || returnnum>foodnum){
				  alert("请输入折扣值在1~"+foodnum+"之间的正整数！");
				  	...
				  	url="./interface/returnonefood.php?billid="+billid+"&foodid="+foodid+"&foodnum="+foodnum+"&cooktype="+cooktype+"&returnnum="+returnnum+"&theday="+theday+"&op=dailysheet";
...
<!-- 										<th class="numeric">其他收入</th> -->
									...
									<?php foreach ($dailyarr as $key=>$val){?>
										...
										<?php if($_SESSION['role']=="manager"){?>
											<td class="numeric"><a href="./interface/delonebill.php?billid=<?php echo $val['billid'];?>&theday=<?php echo $theday;?>&op=dailysheet" onclick="return confirm('确定要删除？');" class="btn mini red"><i class="icon-trash"></i> </a></td> 
										<?php }?>
										</tr>
										<?php }?>
									</tbody>
