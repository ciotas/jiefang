...
class BuyGoodsnotify_NotifyUrl{
	public function addBuyGoodsRecord($inputarr){
		QuDian_InterfaceFactory::createInstancePayDAL()->addBuyGoodsRecord($inputarr);
	}
}
$buygoodsnotifyurl=new BuyGoodsnotify_NotifyUrl();
//计算得出通知验证结果
...
if($verify_result) {//验证成功
	...
	$soldunit=$bodyarr['soldunit'];
	$goodsname=$bodyarr['goodsname'];
	$via=$bodyarr['via'];
	$value=$bodyarr['value'];
...
//     	file_put_contents("/var/www/html/log.txt", json_encode($inputarr));
    	$buygoodsnotifyurl->addBuyGoodsRecord($inputarr);
    }
